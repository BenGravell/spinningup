...
def train(env_name='CartPole-v0', hidden_sizes=None, lr=1e-2, epochs=50, batch_size=5000, optimizer_str='sgd'):
    ...
    if optimizer_str == 'sgd':
        optimizer = SGD(logits_net.parameters(), lr=lr)
    elif optimizer_str == 'mom':
        optimizer = SGD(logits_net.parameters(), lr=lr, momentum=0.9, nesterov=True)
    elif optimizer_str == 'adam':
        optimizer = Adam(logits_net.parameters(), lr=lr)
    elif optimizer_str == 'lbfgs':
        optimizer = LBFGS(logits_net.parameters(), line_search_fn='strong_wolfe', history_size=10)
    else:
...
def make_get_action(logits_net):
    # make function to compute action distribution
    def get_policy(obs):
        logits = logits_net(obs)
        return Categorical(logits=logits)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_name', '--env', type=str, default='gym_ballbeam:BallBeamDiscrete-v0')
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--epochs', type=int, default=40)
    parser.add_argument('--batch_size', type=int, default=1000)
    parser.add_argument('--optimizer_str', type=str, default='adam')
    args = parser.parse_args()
    print('\nUsing simplest formulation of policy gradient.\n')
    trained_net = train(env_name=args.env_name, lr=args.lr, epochs=args.epochs, batch_size=args.batch_size,
                        optimizer_str=args.optimizer_str, hidden_sizes=[4])
